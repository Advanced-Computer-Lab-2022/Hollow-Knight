...
const Report = () => {
  ...
  const handleChange1 = (e) => {
    setState({ Financial: true, Technical: false, Other: false });
  };

  const handleChange2 = (e) => {
    setState({ Financial: false, Technical: true, Other: false });
  };

  const handleChange3 = (e) => {
    setState({ Financial: false, Technical: false, Other: true });
  };
  ...
  const submitreport = async (e) => {
    ...
    if (reason && details) {
      ...
      const response = await fetch(
        `/api/trainees/reportproblem?courseid=${courseid}&&userid=${userid}`,
        {
          method: "POST",
  ...
  return (
    <div>
      <Card sx={{ marginTop: 6 }}>
        ...
        <FormControl>
          {" "}
          <FormLabel sx={{ fontSize: 20, marginLeft: 10 }}>
            {" "}
        ...
        {detailserror && (
          <Typography sx={{ fontSize: 20, marginLeft: 50, color: "red" }}>
